...
# Makefile for building the combined agent (with integrated installer)

.PHONY: all build clean test agent deps build-linux-arm64 build-linux-arm build-linux-riscv64 build-linux-amd64
...
install: agent
	@echo "Installing agent binary to system..."
...
uninstall:
	@echo "Uninstalling agent binary from system..."
...
help:
	...
	@echo "  all                 - Build agent binaries for all architectures (default)"
	@echo "  build               - Build agent binaries for all architectures"
	@echo "  agent               - Build agent binary (local architecture)"
	@echo ""
	...
	@echo "  install             - Install agent binary to system (requires sudo)"
	@echo "  uninstall           - Uninstall agent binary from system (requires sudo)"
	...
	@echo ""
	@echo "Note: The agent now includes integrated installer functionality."
	@echo "      When certificates are missing, it will automatically run the installer flow."
