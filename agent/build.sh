...
show_usage() {
    ...
    echo "  x86_64              Intel/AMD 64-bit (native + musl static)"
    echo "  aarch64             ARM 64-bit (cross-compile + musl static)"
    echo "  armv7l              ARM 32-bit hard float (cross-compile + musl static)"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 --musl                   # musl static build for native architecture"
    echo "  $0 --arch aarch64           # Cross-compile for ARM64 (dynamic linking)"
    echo "  $0 --musl --arch aarch64    # musl static build for ARM64"
    echo "  $0 --musl --arch armv7l     # musl static build for ARM32"
}
